...
class ConstraintViolationComponent(ExplicitComponent):
    ...
    def setup(self):
        for comp in self.constraint_comps:
            if isinstance(comp.constraintComponent, BoundaryBaseComp):
                self.add_input(f'boundaryDistances_{comp.x_key}', val=comp.constraintComponent.zeros)
                self.declare_partials('constraint_violation', f'boundaryDistances_{comp.x_key}')
    ...
    def compute(self, inputs, outputs):
        ...
        for comp in self.constraint_comps:
            if isinstance(comp.constraintComponent, BoundaryBaseComp):
                constraint_violations.append(np.sum(np.minimum(inputs[f'boundaryDistances_{comp.x_key}'], 0) ** 2))
            elif isinstance(comp.constraintComponent, SpacingTypeComp):
    ...
    def compute_partials(self, inputs, partials):
        for comp in self.constraint_comps:
            if isinstance(comp.constraintComponent, BoundaryBaseComp):
                dcdb = np.zeros_like(inputs[f'boundaryDistances_{comp.x_key}'])
                index = np.where(inputs[f'boundaryDistances_{comp.x_key}'] < 0)
                dcdb[index] = 2 * inputs[f'boundaryDistances_{comp.x_key}'][index]
                partials['constraint_violation', f'boundaryDistances_{comp.x_key}'] = dcdb
            elif isinstance(comp.constraintComponent, SpacingTypeComp):

...
class XYBoundaryConstraint(Constraint):
    ...
    def __init__(self, boundary, n_var=1, boundary_type='convex_hull', units=None, relaxation=False, x_key='x', y_key='y', **kwargs):
        ...
        if not hasattr(self, 'boundary_comp'):
            if self.boundary_type == 'polygon':
                self.boundary_comp = PolygonBoundaryComp(
                    n_wt, self.boundary, self.const_id, self.units, self.relaxation, self.x_key, self.y_key)
            elif self.boundary_type == 'multi_polygon':
                self.boundary_comp = MultiPolygonBoundaryComp(n_wt, self.zones, const_id=self.const_id, units=self.units, relaxation=self.relaxation, x_key=self.x_key, y_key=self.y_key)
            elif self.boundary_type == 'turbine_specific':
                self.boundary_comp = TurbineSpecificBoundaryComp(n_wt, self.turbines, self.zones, const_id=self.const_id, units=self.units, relaxation=self.relaxation, x_key=self.x_key, y_key=self.y_key)
            else:
                self.boundary_comp = ConvexBoundaryComp(n_wt, self.boundary, self.boundary_type, self.const_id, self.units, x_key=self.x_key, y_key=self.y_key)
    ...
    def set_design_var_limits(self, design_vars):
        ...
        for k, l, u in zip([self.x_key, self.y_key], bound_min, bound_max):
            if k in design_vars:
    ...
    def _setup(self, problem, group='constraint_group'):
        n_wt = None
        try:
            if self.x_key in problem.design_vars:
                n_wt = len(np.atleast_1d(problem.design_vars[self.x_key][0]))
            elif self.y_key in problem.design_vars:
                n_wt = len(np.atleast_1d(problem.design_vars[self.y_key][0]))
        except Exception:
            pass
        if n_wt is None:
            n_wt = getattr(self, 'n_var', None) or getattr(problem, 'n_wt', 0)
            if not n_wt:
                raise RuntimeError("Cannot infer n_wt. Provide n_var or declare x_key/y_key as design/external vars.")
...
class CircleBoundaryConstraint(XYBoundaryConstraint):
    ...
    def set_design_var_limits(self, design_vars):
        for k, l, u in zip([self.x_key, self.y_key],
...
class BoundaryBaseComp(ConstraintComponent):
    def __init__(self, n_wt, xy_boundary=None, const_id=None, units=None, relaxation=False, x_key='x', y_key='y', **kwargs):
    ...
    def setup(self):
        ...
        self.add_input(self.x_key, np.zeros(self.n_wt),
                       desc='x coordinates of item in global ref. frame', units=self.units)
        self.add_input(self.y_key, np.zeros(self.n_wt),
                       desc='y coordinates of item in global ref. frame', units=self.units)
        if self.relaxation:
    ...
    def compute(self, inputs, outputs):
        ...
        args = {x: inputs[x] for x in [self.x_key, self.y_key, topfarm.type_key] if x in inputs}
        x = inputs[self.x_key]
        y = inputs[self.y_key]
    ...
    def compute_partials(self, inputs, partials):
        ...
        if not self.relaxation:
            dx, dy = self.gradients(**{xy: inputs[k] for xy, k in zip('xy', [self.x_key, self.y_key])})
        else:
...
class ConvexBoundaryComp(BoundaryBaseComp):
    def __init__(self, n_wt, xy_boundary=None, boundary_type='convex_hull', const_id=None, units=None, x_key='x', y_key='y'):
        self.boundary_type = boundary_type
        self.calculate_boundary_and_normals(xy_boundary)
        super().__init__(n_wt, self.xy_boundary, const_id, units,
                         relaxation=False, x_key=x_key, y_key=y_key)
        self.x_key = x_key
        self.y_key = y_key
    ...
    def satisfy(self, state, pad=1.1):
        x, y = [np.asarray(state[xyz], dtype=float) for xyz in [self.x_key, self.y_key]]
...
class PolygonBoundaryComp(BoundaryBaseComp):
    def __init__(self, n_wt, xy_boundary, const_id=None, units=None, relaxation=False, x_key='x', y_key='y'):
        self.nTurbines = n_wt
        self.const_id = const_id
        ...
        super().__init__(n_wt,
                         xy_boundary=self.boundary_properties[0],
                         const_id=const_id, units=units,
                         relaxation=relaxation, x_key=x_key, y_key=y_key)
    ...
    def satisfy(self, state, pad=1.1):
        x, y = [np.asarray(state[xy], dtype=float) for xy in [self.x_key, self.y_key]]
        dist = self.distances(x, y)
...
class Zone(object):
    def __init__(self, boundary, n_var, dist2wt, geometry_type, incl, name, x_key='x', y_key='y'):
        self.name = name
...
class InclusionZone(Zone):
    def __init__(self, boundary, n_var=1, dist2wt=None, geometry_type='polygon', name='', x_key='x', y_key='y'):
        super().__init__(np.asarray(boundary), n_var, dist2wt, geometry_type, incl=1, name=name, x_key=x_key, y_key=y_key)


class ExclusionZone(Zone):
    def __init__(self, boundary, n_var=1, dist2wt=None, geometry_type='polygon', name='', x_key='x', y_key='y'):
        super().__init__(np.asarray(boundary), n_var, dist2wt, geometry_type, incl=0, name=name, x_key=x_key, y_key=y_key)


class MultiPolygonBoundaryComp(PolygonBoundaryComp):
    def __init__(self, n_wt, zones, const_id=None, units=None, relaxation=False, x_key='x', y_key='y', method='nearest',
        ...
        PolygonBoundaryComp.__init__(self, n_wt, xy_boundary=xy_boundaries[0], const_id=const_id, units=units, relaxation=relaxation, x_key=x_key, y_key=y_key)
